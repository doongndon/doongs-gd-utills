...
# v5.24.0
 ...
 - If the achievement row still clips after this, the cause is somewhere this patch can't see, and the next step is comparing exactly which word or count changes right before the point it starts overlapping
...
# v5.3.0
 ...
 - Fixed a crash on the level end screen. It was ours, by a long route. NodeIDs names the nodes on that screen by counting: it walks the children, and for each label whose text starts with "Attempts", "Jumps", "Time" or "Points" it steps a counter on, then uses that counter as an index to find the button menu. We had turned those words into 시도, 점프, 시간, 점수, so the counter stopped short and the name button-menu landed on a label instead. Eclipse then asked for button-menu to hang its cheat indicator on, got a label, and the game died adding a child to it
 - So a translated label now keeps its English. GD stores what a label was told to say separately from what it draws, and getString hands back the stored one - so the patch stores the English and draws the Korean. Every mod that recognises a node by the words on it keeps working, and it is no longer possible for a translation to break one
 - Which also means Tinker's object count line no longer needs the special case from v5.2.0: it reads the label, gets English back, and appends to English
 - 148 more sentences from the mods' source: Globed's settings explanations and every confirm dialog it has, Level Thumbnails' entire set of refusals and badges, Object Workshop's upload rules and its two authentication explainers, PlatformerSaves' low detail warnings, QOLMod's two cheat warnings, and Improved Playtest's camera menu
...
# v5.1.0
 ...
 - Korean no longer spills out of buttons. A button is cut to fit the English that was in it, and Korean often writes the same thing wider, so the letters ran past the edge. The patch now measures the English before swapping and shrinks the Korean to that width if it would overflow - smaller letters, but inside the button. GD does the same to its own text
 - Text built one line at a time is translated line by line. BetterInfo's level record has an optional line for orbs, for the leaderboard, for the editor, so the whole block never matched anything as one string. 21 of its lines are now in the table: 총 시도, 총 점프, 클릭 (최고 시도), 시간 (최고 시도), 진행 기록
